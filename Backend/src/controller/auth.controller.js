...
const userLogin = asyncHandler(async (req, res) => {
  ...
  const { accessToken, refrashToken } = await generateAccessAndRefereshTokens(
    ...
    const loggedInUser = await userModel.findById(user._id).select("-password -refrashToken");
    const option = {
    httpOnly: true,
...
const refrashAcessToken = asyncHandler(async  (req, res) => {
  try {
    const incomingRecreachToken = req.cookies.refrashToken || req.body.refrashToken
    if(!incomingRecreachToken){
      throw new ApiError(401, "Unauthorized request") 
    }
    const decodedToken = jwt.verify(incomingRecreachToken, process.env.REFRESH_TOKEN_SECRET)
    ...
    let {accessToken, newRefrashToken} = generateAccessAndRefereshTokens(user._id)
    ...
    json(new ApiResponse(200,{accessToken, refrashToken: newRefrashToken},"your route is working")) 
  } catch (error) {
